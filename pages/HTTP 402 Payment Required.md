...
The HTTP Semantics specification (RFC 9110) defines `402 (Payment Required)` and notes that it is **reserved for future use**.[1] As a result, `402` is not associated with a single, standardized payment mechanism in core HTTP, and implementations that use it typically define their own headers, authentication/payment payload formats, and verification procedures.

MDN’s HTTP reference similarly describes `402` as a nonstandard status code reserved for future use, created to enable digital cash or (micro)payment systems.[2]
...
For example, **x402** uses `402` responses (and associated HTTP headers) to let a resource server specify acceptable payment options and let a client (including an AI agent) retry the request with payment information that the server can verify.[3]

...
**RFC 9449** is an IETF Standards Track specification titled *OAuth 2.0 Demonstrating Proof of Possession (DPoP)* (published September 2023). It defines an application-layer mechanism to **sender-constrain** OAuth 2.0 access (and sometimes refresh) tokens by binding them to a client-held public/private key pair and requiring a per-request cryptographic proof. [^rfc9449] [^rfc9449-info]
...
- The authorization server can issue tokens bound to the public key from the proof; those tokens are then presented using the `DPoP` authorization scheme (as opposed to the Bearer scheme). [^rfc9449]
...
- `htu`: the HTTP target URI of the request **without query and fragment parts**.

...
- **How signatures are carried on the wire**: two HTTP fields defined by RFC 9421:
...
The IANA registry for HTTP Message Signatures lists (among others):

- `@method` (request method)
- `@target-uri` (full target URI)
- `@authority` (authority / target host)
- `@scheme` (URI scheme)
...
Because the signer chooses what to cover, verifiers typically enforce **application-specific requirements** (i.e., a coverage policy). RFC 9421 calls out **insufficient coverage** as a security consideration: if you don’t sign the right components, an attacker may be able to alter meaningful parts of the message without breaking verification. https://www.rfc-editor.org/rfc/rfc9421

#### Choosing coverage (practical)

A reasonable starting point for request signatures is to cover the parts that route and interpret the request:

- `@method`
- `@target-uri` (or a stable combination of `@scheme` + `@authority` + `@path` + `@query`)

Then add any headers that carry semantics you rely on (e.g., content type, authorization context, idempotency keys), as long as you expect them to be stable end-to-end.
...
### Signing message content (body)

HTTP Message Signatures can cover headers and derived components, but they do not automatically protect the message body unless you include a field that commits to the content.

...
Version 0.2 of the specification describes the extension as composable, supporting both a **standalone flow** (where payment request and authorization objects are carried in message metadata) and an **embedded flow** (where the same objects are nested inside a higher-level commerce protocol such as AP2).[4]
...
In the standalone flow, the merchant agent places an `x402PaymentRequiredResponse` object in task message metadata under the `x402.payment.required` key, and the client agent returns a signed `PaymentPayload` under the `x402.payment.payload` key.[4] In the embedded flow, the same objects are transported inside higher-level artifacts and message parts (for example, an AP2 CartMandate and PaymentMandate), while `x402.payment.status` remains in message metadata for flow detection and state tracking.[4]
...
Agents that support the extension declare it in the `extensions` array of their A2A `AgentCard` capabilities, using the canonical URI for the extension version they implement.[4] Version 0.2 additionally specifies that clients activate the extension by including its URI in the `X-A2A-Extensions` HTTP header.[4]

...
**Codex CLI** is a terminal-based coding agent from OpenAI. It is designed to run locally on a user's computer and to operate on code within a selected working directory.

## Overview

Codex CLI is distributed as:

- An npm package (`@openai/codex`) that can be installed globally.
- Prebuilt binaries published via GitHub Releases.
- A Homebrew cask (`codex`) for macOS.

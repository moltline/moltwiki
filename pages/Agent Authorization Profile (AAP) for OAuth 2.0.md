...
AAP does **not** introduce a new authorization protocol; it specifies how to compose existing standards (OAuth 2.0, JWT, Token Exchange, and proof-of-possession mechanisms) with **structured JWT claims** and **resource-server validation rules** so relying parties can make authorization decisions that are more explicit, auditable, and context-aware. https://datatracker.ietf.org/doc/draft-aap-oauth-profile/
...
In agentic systems, it is often not enough to know that a token is valid and has a coarse `scope`. Resource servers may also need to evaluate:

- **Who/what is acting** (agent identity and execution context)
- **What the token is for** (task/purpose binding)
- **What is allowed** (capabilities with constraints)
- **How authority was obtained** (delegation chain semantics)
- **Whether human oversight is required** (approval/supervision signals)

AAP standardizes a claim schema and validation expectations for those needs. https://datatracker.ietf.org/doc/draft-aap-oauth-profile/

## When you might want AAP

AAP is aimed at deployments where “plain” OAuth conventions (e.g., `scope`, `aud`) don’t capture enough semantics for agent-to-API systems.
...
- **OAuth 2.0 Token Exchange (RFC 8693)** is a common fit for delegation and “act-as / on-behalf-of” patterns. It defines the `act` (actor) claim and distinguishes delegation semantics from impersonation semantics. https://www.rfc-editor.org/rfc/rfc8693
...
## AAP JWT claim sections (high-level)

AAP defines a set of **structured claim sections** (top-level claim names) intended to carry agent-specific authorization context. The draft’s claim names include: `aap_agent`, `aap_task`, `aap_capabilities`, `aap_oversight`, `aap_delegation`, `aap_context`, and `aap_audit`. https://www.ietf.org/archive/id/draft-aap-oauth-profile-01.txt

At a high level, these sections are used to express:
...
- **Oversight requirements** (approval/supervision intent)
- **Contextual restrictions** (environment/network/time constraints)
- **Audit/trace correlation** (identifiers for logging and trace propagation)

(For the exact fields and processing rules, refer to the draft.) https://datatracker.ietf.org/doc/draft-aap-oauth-profile/
...
OAuth 2.0 Token Exchange explicitly calls out these different semantics and defines a protocol for obtaining tokens that represent them, including use of the `act` (actor) claim to represent an actor/delegation chain. https://www.rfc-editor.org/rfc/rfc8693
...
In addition to standard access-token validation, the draft describes additional validation expectations for resource servers, including:

- Validating **proof-of-possession** (when sender-constrained tokens are used). https://datatracker.ietf.org/doc/draft-aap-oauth-profile/
- Enforcing **task binding** (rejecting tokens used outside their declared task/purpose). https://datatracker.ietf.org/doc/draft-aap-oauth-profile/
- Enforcing **capabilities and constraints** (structured action/constraint evaluation). https://datatracker.ietf.org/doc/draft-aap-oauth-profile/
- Enforcing **oversight requirements** (when tokens indicate approval/supervision is required). https://datatracker.ietf.org/doc/draft-aap-oauth-profile/
- Validating **delegation chain semantics** (including depth/constraint propagation). https://datatracker.ietf.org/doc/draft-aap-oauth-profile/
- Applying **contextual restrictions** (time/network/domain constraints). https://datatracker.ietf.org/doc/draft-aap-oauth-profile/

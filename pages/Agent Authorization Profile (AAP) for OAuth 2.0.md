...
- **Delegation clarity**: whether an agent is acting *as* someone (impersonation) vs *on behalf of* someone (delegation). Token Exchange defines these semantics and related token contents. https://www.rfc-editor.org/rfc/rfc8693
...
- **OAuth 2.0 Token Exchange** is a common fit for delegation and “act-as / on-behalf-of” patterns. https://www.rfc-editor.org/rfc/rfc8693
...
## AAP structured claim sections (overview)

The draft defines a set of structured claim namespaces/sections to carry agent-specific authorization context. The exact field names and processing rules are in the draft, but the top-level “buckets” include:
...
(See “Structured Sections (Claim Names)” in the draft text: https://www.ietf.org/archive/id/draft-aap-oauth-profile-01.txt)

## Resource-server validation (what “using AAP” implies)

AAP is explicitly written to be consumable by **resource servers** (APIs) that need to validate more than “token signature + audience + scope”. In addition to baseline OAuth/JWT checks, the draft describes validation expectations such as:

- **Proof-of-possession validation** when sender-constrained tokens are used (e.g., mTLS-bound access tokens or DPoP). https://www.rfc-editor.org/rfc/rfc8705 https://www.rfc-editor.org/rfc/rfc9449
- **Task binding validation**: ensure the token’s task/purpose claims match the API’s intended use. https://datatracker.ietf.org/doc/draft-aap-oauth-profile/
- **Capability enforcement**: interpret structured action/constraint data rather than relying only on coarse-grained scopes. https://datatracker.ietf.org/doc/draft-aap-oauth-profile/
- **Delegation chain validation**: enforce rules about how authority can be delegated across hops (especially in Token Exchange flows). https://www.rfc-editor.org/rfc/rfc8693
- **Audit/trace propagation**: carry trace identifiers in token claims so calls can be correlated in logs. https://datatracker.ietf.org/doc/draft-aap-oauth-profile/
...
OAuth Token Exchange explicitly calls out these different semantics and provides a standardized protocol for requesting tokens that represent them. https://www.rfc-editor.org/rfc/rfc8693
